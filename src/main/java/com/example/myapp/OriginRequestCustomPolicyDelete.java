...
/**
 * CloudFront 自定义源请求策略删除示例。
 *
 * 使用与创建示例相同的默认策略名称删除源请求策略。
 *
 ...
 */
public class OriginRequestCustomPolicyDelete {
    ...
    public static void main(String[] args) {
        ...
        try (CloudFrontClient cloudFrontClient = CloudFrontClient.builder()
            ...
            DeleteOriginRequestPolicyRequest deleteRequest = DeleteOriginRequestPolicyRequest.builder()
                    .id(policyId)
                    .ifMatch(eTag)
                    .build();

            cloudFrontClient.deleteOriginRequestPolicy(deleteRequest);
            ...
            if (details != null) {
                System.err.println("调用 CloudFront 删除源请求策略失败: " + nullSafe(details.errorMessage()));
                System.err.println("错误码: " + nullSafe(details.errorCode()));
            } else {
                System.err.println("调用 CloudFront 删除源请求策略失败: " + ex.getMessage());
            }
        } catch (Exception ex) {

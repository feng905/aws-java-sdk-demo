...
/**
 * CloudFront 自定义缓存策略删除示例。
 *
 * 使用与创建示例相同的默认策略名称删除缓存策略。
 *
 ...
 */
public class CacheCustomPolicyDelete {
    ...
    public static void main(String[] args) {
        ...
        try (CloudFrontClient cloudFrontClient = CloudFrontClient.builder()
            ...
            DeleteCachePolicyRequest deleteRequest = DeleteCachePolicyRequest.builder()
                    .id(policyId)
                    .ifMatch(eTag)
                    .build();

            cloudFrontClient.deleteCachePolicy(deleteRequest);
            ...
            if (details != null) {
                System.err.println("调用 CloudFront 删除缓存策略失败: " + nullSafe(details.errorMessage()));
                System.err.println("错误码: " + nullSafe(details.errorCode()));
            } else {
                System.err.println("调用 CloudFront 删除缓存策略失败: " + ex.getMessage());
            }
        } catch (Exception ex) {
